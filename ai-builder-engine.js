...
// Safe Builder bridge: applies saved AI intent when index.html loads.
export function applySavedBuilderIntent({typeEl,featureMap={},onApplied}={}){
 try{
  ...
  if(typeEl){
   ...
   if([...typeEl.options].some(o=>o.value===target)) typeEl.value=target;
  }
  (intent.features||[]).forEach(key=>{const id=featureMap[key];const el=id&&document.getElementById(id);if(el)el.checked=true;});
  localStorage.removeItem('nurjahanBuilderIntent');
  if(typeof onApplied==='function')onApplied(intent);
  return true;
 }catch(e){console.warn('Nurjahan Builder intent skipped',e);localStorage.removeItem('nurjahanBuilderIntent');return false;}
}
